package prob_202502.week2.crossline_17386;
...
public class Main {
    ...
    // 수직 벡터를 구하는 알고리즘
    // 두 벡터 A와 B에 동시에 수직인 벡터는 A와 B를 외적해서 얻을 수 있다.
    public static BigInteger CCW(Point A, Point B, Point C) {
        // v1(a1, b1), v2(a2, b2)의 외적 = a1b2 - a2b1 (xy 좌표평면 기준)
        // 두 벡터가 평행하다면 -> 외적 역시 같은 2차원 평면 상으로 나타나므로 외적값은 0이 된다.
        // 두 벡터가 평행하지 않다면 -> 두 벡터에 동시에 수직인 벡터는 z축에 존재한다. 즉 음수 혹은 양수 값이 됨.
        // 반시계방향 -> 양수, 시계방향 -> 음수, 평행 -> 0
        // 3차원 벡터로 치면 (0, 0, z)가 되는 식.
