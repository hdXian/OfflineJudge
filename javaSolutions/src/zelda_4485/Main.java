...
//            matrix = new int[N][N];
            ...
            // 끝 노드는 (N * N - 1)번
            System.out.println("minCost[N*N-1] = " + minCost[N*N-1]);
    ...
    public static void updateNeighbor(int num) {
        System.out.println("in updateNei num = " + num);
        ...
        if (num%N != (N-1)) {
            int idx = num+1; // 목적지
            System.out.println("오른쪽 조건 가능 - idx = " + idx);
            minCost[idx] = Math.min(minCost[idx], minCost[num] + nodes[idx]);
            System.out.printf("minCost[%d] = %d\n", idx, minCost[idx]);
        }

        // 아래쪽 안되는 조건 -> 몫이 N 이상일 때
        if (num / N < N-1) {
            int idx = num + N; // 목적지
            System.out.println("아래쪽 조건 가능 - idx = " + idx);
            minCost[idx] = Math.min(minCost[idx], minCost[num] + nodes[idx]);
            System.out.printf("minCost[%d] = %d\n", idx, minCost[idx]);
        }
    }
